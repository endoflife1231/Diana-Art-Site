...
description: Debug and verify web applications in a live Chrome browser through Chrome
  DevTools MCP. Use for DOM, console, network, screenshot, accessibility, or performance
  inspection.
metadata:
  author: Addy Osmani; Codex migration by OpenAI
  license: MIT
...
Codex CLI and the IDE extension share MCP configuration. Prefer an isolated browser profile for automated testing:

```bash
codex mcp add chrome_devtools -- npx -y chrome-devtools-mcp@latest --isolated
```

Equivalent project-scoped `.codex/config.toml` entry for a trusted repository:
...
Verify the server with `codex mcp --help` and, in the Codex TUI, `/mcp`. If the server is unavailable, use `$playwright` or `$playwright-interactive` instead of pretending browser evidence was collected.

`--isolated` uses a temporary Chrome profile. Avoid attaching to a personal browser profile unless the user explicitly needs authenticated state and understands that open tabs, cookies, and page content become available to the MCP server.
